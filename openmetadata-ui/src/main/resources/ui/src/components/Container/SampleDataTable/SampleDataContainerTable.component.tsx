...
const SampleDataContainer = ({
  ...
  const getSampleDataWithType = (container: Container) => {
    const { sampleData, dataModel } = container;
    const updatedColumns = sampleData?.columns?.map((column) => {
      const matchedColumn = dataModel?.columns.find((col) => col.name === column);
  ...
  const fetchSampleData = async () => {
    try {
      const tableData = await getSampleDataByContainerId(containerId);
  ...
  const handleDeleteSampleData = async () => {
    try {
      await deleteSampleDataByContainerId(containerId);
      handleDeleteModal();
      fetchSampleData();
    } catch (error) {
      showErrorToast(
  ...
  useEffect(() => {
    setIsLoading(true);
    if (!isDeleted && containerId && !isTourPage) {
      fetchSampleData();
    } else {
      setIsLoading(false);
    }
...
export default withLoader<SampleDataProps>(SampleDataContainer);
