...
import {
  getLatestContainerProfileByFqn,
  getContainerDetailsByFQN,
} from '../../../../rest/containerAPI';
...
export const TableProfilerProvider = ({
  ...
  const [tableProfiler, setTableProfiler] = useState<Container>();
  // customMetric is fetch from table api and has response type of Table
  const [customMetric, setCustomMetric] = useState<Container>();
  ...
  const isDeleted = useMemo(() => container?.deleted, [container]);
  ...
  const overallSummary: OverallTableSummaryType[] = useMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        value: profile?.columnCount ?? tableProfiler?.dataModel?.columns.length ?? 0,
      },
      {
  ...
  const handleUpdateCustomMetrics = (customMetric: Container) => {
  ...
  const fetchLatestProfilerData = async () => {
    ...
    try {
      const profiler = await getLatestContainerProfileByFqn(datasetFQN);
      const customMetricResponse = await getContainerDetailsByFQN(datasetFQN, {
        fields: 'customMetrics,dataModel',
      });
  ...
  useEffect(() => {
    ...
    if (isTourOpen) {
      setTableProfiler(mockDatasetData.tableDetails as unknown as Container);
    }
    ...
    <TableProfilerContext.Provider value={tableProfilerPropsData}>
      ...
      {settingModalVisible && (
        <ProfilerSettingsModal
          columns={tableProfiler?.dataModel?.columns ?? []}
          containerId={tableProfiler?.id ?? ''}
